...
async function bootstrap() {
  ...
  app.post("/api/agents/create", async (req, res) => {
    ...
    try {
      ...
      if (!simulation.running) simulation.start(8000);

      res.json({
        success: true,
        agent: { name: agent.name, type: agent.kind, wallet: agent.walletAddress, personality, strategy },
        chain
      });
    } catch (error) {
  ...
  app.post("/api/agents/create-external", async (req, res) => {
    ...
    try {
      ...
      if (!probe.ok && strictCompatibility) return badRequest(res, `external endpoint compatibility probe failed: ${probe.reason}`);
      ...
      res.json({
        success: true,
        agent: { name: agent.name, type: agent.kind, wallet: agent.walletAddress, endpoint },
        chain
      });
    } catch (error) {
      return badRequest(res, error.message);
  ...
  app.post("/api/agents/create-llm", async (req, res) => {
    ...
    try {
      ...
      res.json({
        success: true,
        agent: { name: agent.name, type: agent.kind, wallet: agent.walletAddress, provider, model },
        chain
      });
    } catch (error) {
      return badRequest(res, error.message);
  ...
  app.post("/api/agents/probe-external", async (req, res) => {
    ...
    if (!endpoint) return badRequest(res, "agent_endpoint is required");
    const probe = await probeExternalEndpoint(endpoint, apiKey);
    res.json({ success: probe.ok, protocolVersion: MASP_PROTOCOL_VERSION, probe });
  });
  ...
  app.post("/api/agents/remove", async (req, res) => {
    const name = normalizeName(req.body?.name);
    if (!name) return badRequest(res, "name is required");
    const removed = await simulation.removeAgentByName(name);
  ...
  app.post("/api/feed/clear", async (_, res) => {
    await simulation.clearFeed();
    res.json({ success: true });
  });

  app.post("/api/feed/remove-fallbacks", async (_, res) => {
    const removed = await simulation.removeFallbackFeedEntries();
  ...
  app.listen(port, () => {
    console.log(`Bootstrap: MASP server listening on http://localhost:${port}`);
